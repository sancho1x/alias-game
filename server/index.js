...
const rooms = {};
const MAX_ROOMS = 100; 
const ROOM_TIMEOUT = 2 * 60 * 60 * 1000; // 2 години інактивації
...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', ({ playerName, playerId }) => {
    if (Object.keys(rooms).length >= MAX_ROOMS) return socket.emit('error', 'Ліміт кімнат!');
    ...
    rooms[roomCode] = {
      ...
      gameState: { status: 'lobby', currentTeamIndex: 0, explainerIndices: {}, currentWord: '', timeLeft: 60, usedWords: [], roundHistory: [] },
      timerInterval: null
    };
  ...
  socket.on('joinRoom', ({ roomCode, playerName, playerId }) => {
    ...
    if (room) {
      ...
      if (existing) {
        existing.id = socket.id; existing.name = playerName; existing.online = true;
      } else {
        room.players.push({ id: socket.id, playerId, name: playerName, teamId: null, online: true });
      }
      if (!room.hostId || !room.players.find(p => p.playerId === room.hostId && p.online)) room.hostId = playerId;
      socket.join(roomCode);
      io.to(roomCode).emit('roomUpdated', getSafeRoom(room));
    } else socket.emit('error', 'Кімнату не знайдено.');
  });
  ...
  socket.on('adjustScore', ({ roomCode, teamId, amount }) => {
    ...
    if (room && player && room.hostId === player.playerId) {
      ...
      if (team) { team.score += amount; io.to(roomCode).emit('roomUpdated', getSafeRoom(room)); }
    }
  });
    ...
    for (const code in rooms) {
      ...
      if (player) {
        ...
        if (room.hostId === player.playerId) {
          const next = room.players.find(p => p.playerId !== player.playerId && p.online);
  ...
  socket.on('joinTeam', ({ roomCode, teamId }) => {
    ...
    if (room) {
      touchRoom(roomCode);
      const player = room.players.find(p => p.id === socket.id);
      if (player && player.teamId !== teamId && room.players.filter(p => p.teamId === teamId).length >= 2) return socket.emit('error', 'Команда повна');
  ...
  socket.on('startTurn', ({ roomCode }) => {
    ...
    for (const team of room.teams) if (room.players.filter(p => p.teamId === team.id).length < 2) return socket.emit('error', `Мін. 2 гравці в ${team.name}`);
    ...
    room.gameState.currentWord = (available = (dictionaries[room.settings.dictType] || dictionaries.easy).filter(w => !room.gameState.usedWords.includes(w))).length === 0 ? (room.gameState.usedWords = [], dictionaries[room.settings.dictType][0]) : available[Math.floor(Math.random() * available.length)];
    room.gameState.usedWords.push(room.gameState.currentWord);
    io.to(roomCode).emit('roomUpdated', getSafeRoom(room));
    ...
    room.timerInterval = setInterval(() => {
      ...
      if (room.gameState.timeLeft <= 0) {
        clearInterval(room.timerInterval); room.gameState.status = 'last_word'; io.to(roomCode).emit('roomUpdated', getSafeRoom(room));
      }
    }, 1000);
  });
  ...
  socket.on('nextWord', ({ roomCode, isCorrect }) => {
    const room = rooms[roomCode];
    if (room && room.gameState.status === 'playing' && room.gameState.currentExplainerId === socket.id) {
      touchRoom(roomCode);
      const team = room.teams.find(t => t.id === room.gameState.currentTeamId);
      if (isCorrect) team.score += 1; else team.score -= 1;
      room.gameState.roundHistory.push({ word: room.gameState.currentWord, isCorrect });
      const pool = dictionaries[room.settings.dictType];
      let av = pool.filter(w => !room.gameState.usedWords.includes(w));
      if (av.length === 0) { room.gameState.usedWords = []; av = pool; }
      room.gameState.currentWord = av[Math.floor(Math.random() * av.length)];
      room.gameState.usedWords.push(room.gameState.currentWord);
      io.to(roomCode).emit('roomUpdated', getSafeRoom(room));
    }
  });
  ...
  socket.on('lastWordResult', ({ roomCode, isCorrect }) => {
    const room = rooms[roomCode];
    if (room && room.gameState.status === 'last_word' && room.gameState.currentExplainerId === socket.id) {
      touchRoom(roomCode);
      const team = room.teams.find(t => t.id === room.gameState.currentTeamId);
  ...
  socket.on('endGame', ({ roomCode }) => {
    ...
    const player = room?.players.find(p => p.id === socket.id);
    if (room && player && room.hostId === player.playerId) {
      touchRoom(roomCode); if (room.timerInterval) clearInterval(room.timerInterval);
      room.gameState.status = 'lobby'; io.to(roomCode).emit('roomUpdated', getSafeRoom(room));
    }
  });
});
