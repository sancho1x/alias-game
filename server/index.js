...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', ({ playerName, playerId }) => {
    if (Object.keys(rooms).length >= MAX_ROOMS) {
      return socket.emit('error', 'Сервери перевантажені!');
    }
  ...
  socket.on('startTurn', ({ roomCode }) => {
    ...
    // ВИЗНАЧЕННЯ ВЕДУЧОГО: беремо індекс і крутимо його
  ...
  socket.on('lastWordResult', ({ roomCode, isCorrect }) => {
    ...
    if (room && room.gameState.status === 'last_word') {
      ...
      // ТУТ ВАЖЛИВО: Оновлюємо індекс для наступного разу, щоб пояснював інший гравець
