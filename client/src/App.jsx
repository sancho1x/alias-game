...
// ТУТ МАЄ БУТИ ПОСИЛАННЯ НА ВАШ RENDER СЕРВЕР
...
function App() {
  ...
  const handleAdjustScore = (teamId, amount) => {
    socket.emit('adjustScore', { roomCode: room.id, teamId, amount });
  };
  ...
  if (room.gameState.status === 'countdown') {
    return (
      <div className="app-wrapper game-mode" style={{ justifyContent: 'center', alignItems: 'center' }}>
        ...
        <h1 style={{ fontSize: '10rem', color: 'var(--accent-yellow)', margin: '0' }}>
          {localTimer}
        </h1>
      </div>
    );
    ...
    return (
      <div className="app-wrapper">
        <div className="container end-turn-container">
          <h1 className="text-danger" style={{ textAlign: 'center', marginBottom: '20px' }}>Хід завершено! 🏁</h1>
          ...
          <div className="score-board">
            <h3 style={{ marginBottom: '15px' }}>Поточний рахунок:</h3>
          ...
          <div className="next-team-announcement">
            ...
            <p className="muted" style={{ marginTop: '10px' }}>
              Відгадує: <strong>{nextGuesser?.name || '...'}</strong>
            </p>
          </div>
          ...
          {isHost ? (
            ...
            <p className="muted" style={{ marginTop: '20px', textAlign: 'center' }}>Очікуємо, поки хост запустить раунд...</p>
          )}
        </div>
      </div>
    );
    ...
    <div className="app-wrapper">
      <div className="container">
        ...
        <div className="settings-panel">
          ...
          {isHost ? (
            ...
            <div className="read-only-settings" style={{ marginTop: '15px' }}>
              <p style={{ marginBottom: '10px' }}>Час раунду: <strong>{room.settings.timer} сек</strong></p>
              <p>Словник: <strong>{
                ...
                <div key={t.id} className="team-card">
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
                      {isHost && (
                        <button className="score-adjust" onClick={() => handleAdjustScore(t.id, -1)}>-</button>
                      )}
        ...
        <div className="players-list">
          <h3>Гравці</h3>
          <ul style={{ marginTop: '15px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {room.players.map(p => (
